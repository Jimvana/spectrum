...
def tokenize_text(source: str) -> list[str]:
    ...
    while i < n:
        ...
        if ch.isalpha():
            ...
            if word_lower in D.TOKEN_TO_SPEC_ID:
                if cap_tok:
                    tokens.append(cap_tok)
                tokens.append(word_lower)
            else:
                # Spell it out letter by letter between BEGIN/END markers
                if cap_tok:
                    tokens.append(cap_tok)
                tokens.append(T_BEGIN_WORD)
                for c in word_lower:
                    tokens.append(c)
                tokens.append(T_END_WORD)
            continue

        # ── Punctuation / symbols ────────────────────────────────────────────
        # Try 3-char match first (e.g. "..." ellipsis, "===" JS strict equal)
        if i + 2 < n and source[i:i+3] in D.TOKEN_TO_SPEC_ID:
            tokens.append(source[i:i+3])
            i += 3
            continue

        # Try 2-char match
        if i + 1 < n and source[i:i+2] in D.TOKEN_TO_SPEC_ID:
